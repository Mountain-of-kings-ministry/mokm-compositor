...
Item {
    ...
    property int menuTargetTrackIndex: -1

    // Pixels per frame for timeline scaling
    property double pixelsPerFrame: 2.0

    // Top-level helper for formatting timecodes (available anywhere in this item)
                        ...
                        ListView {
                            ...
                            delegate: ItemDelegate {
                                ...
                                contentItem: RowLayout {
                                    Image {
                                        source: binDelegate.model.isFolder ? "../icons/outline/folder.svg" : "image://media-bin/" + binDelegate.model.id
                                        fillMode: Image.PreserveAspectCrop
                                        width: 24
                                        height: 24
                                        opacity: binDelegate.model.isFolder ? 1.0 : 1.0
                                    }
                                    Text {
                                ...
                                onDoubleClicked: {
                                    if (binDelegate.model.isFolder)
                                        MediaBinModel.currentParentId = binDelegate.model.id;
                                }
                                ...
                                MouseArea {
                                    ...
                                    Image {
                                        ...
                                        source: "image://media-bin/" + binDelegate.model.id
                                        width: 32
                                        height: 20
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    Rectangle {
                                        ...
                                        delegate: RowLayout {
                                            ...
                                            Rectangle {
                                                ...
                                                DropArea {
                                                    ...
                                                    onDropped: drop => {
                                                        if (drop.hasUrls) {
                                                            // File from OS: import then add to this track at playhead
                                                            for (var i = 0; i < drop.urls.length; ++i) {
                                                                var p = drop.urls[i];
                                                                MediaBinModel.importMedia(p);
                                                                // try to find the imported item by filePath
                                                                var all = MediaBinModel.getAllMedia();
                                                                var foundId = null;
                                                                for (var j = 0; j < all.length; ++j) {
                                                                    if (all[j].filePath === p || all[j].filePath === p.toString()) {
                                                                        foundId = all[j].id;
                                                                        break;
                                                                    }
                                                                }
                                                                if (foundId) {
                                                                    var durationSecs = 0;
                                                                    var media = MediaBinModel.getMediaById(foundId);
                                                                    if (media.durationSeconds)
                                                                        durationSecs = media.durationSeconds;
                                                                    var fps = PlayheadController.fps || 24.0;
                                                                    var durationFrames = Math.max(1, Math.floor(durationSecs * fps));
                                                                    var dropFrame = Math.max(0, Math.floor(drop.x / videoEditorWorkspace.pixelsPerFrame));
                                                                    Mokm.Core.UndoManager.addClip(trackRow.index, foundId, media.filename || "Clip", dropFrame, durationFrames);
                                                                }
                                                            }
                                                        } else {
                                                ...
                                                Menu {
                                                    ...
                                                    Action {
                                                        ...
                                                        onTriggered: {
                                                            videoEditorWorkspace.menuTargetTrackIndex = trackRow.index;
                                                            importFromBinDialog.open();
                                                        }
                                                    }
                                                    Action {
                        ...
                        Dialog {
                            id: importFromBinDialog
                            modal: true
                            title: "Import from Bin"
                            ...
                            contentItem: ColumnLayout {
                                ...
                                ListView {
                                    ...
                                    model: MediaBinModel
                                    delegate: ItemDelegate {
                                        width: importBinList.width
                                        height: 44
                                        RowLayout {
                                            Image {
                                                source: model.isFolder ? "../icons/outline/folder.svg" : "image://media-bin/" + model.id
                                                width: 36
                                                height: 36
                                                fillMode: Image.PreserveAspectFit
                                            }
                                        ...
                                        onDoubleClicked: {
                                            ...
                                            var mediaName = model.filename;
                                            var durationSecs = parseFloat(model.durationSeconds || 0);
                                            var fps = PlayheadController.fps || 24.0;
                                            var durationFrames = Math.max(1, Math.floor(durationSecs * fps));
                                            var dropFrame = Math.max(0, PlayheadController.currentFrame);
                                            Mokm.Core.UndoManager.addClip(videoEditorWorkspace.menuTargetTrackIndex, mediaId, mediaName, dropFrame, durationFrames);
                                            importFromBinDialog.close();
                                        }
                                    }
                                }
                            }
